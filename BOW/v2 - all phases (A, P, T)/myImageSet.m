%myImageSet Define collection of images.
%
%   -----------------------------------------------------------------------
%   Use imageDatastore, which offers more functionality and is recommended
%   instead of myImageSet.
%   ----------------------------------------------------------------------- 
%
%   imgSet = myImageSet(imageLocation) defines a collection of images by
...
%
%   imgSetVector = myImageSet(imgFolder, 'recursive') travels the folder structure
...
%
%   myImageSet properties:
%      Description    - Information about the myImageSet
...
%
%   imgSets = myImageSet(imgFolder,'recursive')
...
classdef myImageSet
    ...
    methods (Access = private)
        ...
        function this = parseDirInput(this, varargin)
            ...
            dirName = varargin{1};
            dirName = myImageSet.expandDirToAbsPath(dirName);
            ...
            if ~isdir(dirName)
                ...
                % scalar
                error(message('vision:myImageSet:inputMustBeValidFolderName'));
            end
            
            if isRecursive
                % pre-pend the root folder and fetch all subdirectories
                allDirs = [{dirName}, myImageSet.getSubDirNames(dirName)];
                
                n = numel(allDirs);
                
                this(n) = myImageSet.makeDefaultObject(this); % pre-allocate object array
                for i=1:n
                ...
                this.ImageLocation = myImageSet.parseFolder(dirName);
        ...
        function this = parseCellInput(this, in)
            ...
            if  ~areAllFilenamesValid
                error(message('vision:myImageSet:allFilenamesMustBeStrings'));
            end
            
            % verify that all files exist
            areAllFilenamesValid = all(cellfun(@(x) (exist(x,'file')==2), in));
            if  ~areAllFilenamesValid
                error(message('vision:myImageSet:allFilesMustExist'));
            end
            ...
            this.ImageLocation = myImageSet.expandFileToAbsPath(in);
        ...
        function subDirNames = getSubDirNames(rootDir)
            ...
            if ~isempty(subDirNames)
                for i=1:numel(subDirNames)
                    tempDirNames = myImageSet.getSubDirNames(subDirNames{i});
    ...
    methods (Access = public)
        ...
        %------------------------------------------------------------------
        function this = myImageSet(varargin)
            
            narginchk(0,2); % cell or dir or dir+'recursive'
            
            % myImageSet constructor
        ...
        function imOut = read(this, index)
            ...
            if ~isscalar(this)
                error(message('vision:myImageSet:methodNotSupportedForArrays'));
            end
            ...
            try
                ...
                % set could be empty
                if this.Count == 0
                    error(message('vision:myImageSet:setIsEmpty'));
                end
                
                % indicate which file could not be read, in addition to
                % the error message from imread
                error(message('vision:myImageSet:couldNotReadFile',...
    ...
    methods(Access = public, Sealed)
        ...
        function out = select(this, index)
            ...
            %  imgSetOut = select(imgSet, idx) returns a new myImageSet,
            ...
            if n > 1
                if isanyempty(this)
                    error(message('vision:myImageSet:arrayContainsEmptyImageSet'));
                end
            else
            ...
            out(n) = myImageSet.makeDefaultObject(this);
        ...
        function varargout = partition(this, groupSizes, varargin)
            ...
            if isSplitInPercent
                if sum(groupSizes) > 1
                    error(message('vision:myImageSet:invalidPercentageSplitRequest'));
                end
            else
                if  any(sum(groupSizes) > [this.Count])
                    error(message('vision:myImageSet:invalidSplitRequest'));
                end
            end
            ...
            for setIdx = 1:numSets;
                ...
                for splitIdx = 1:numSplits
                    ...
                    % make a split
                    if isempty(range)
                        varargout{splitIdx} = [varargout{splitIdx}, myImageSet.makeDefaultObject(this)];
                ...
                if isAnySplitEmpty
                    warning(message('vision:myImageSet:atLeastOneOutputEmpty'));
                end
    ...
    methods(Access = protected)
        ...
        % input myImageSet, imgSet, that contains a subset of the property
        ...
        % myImageSet should overload this method to ensure that the SELECT
